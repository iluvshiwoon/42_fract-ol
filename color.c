...
/*   Updated: 2024/04/05 10:58:23 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "fractol.h"
...
void	build_palette(t_vars *vars)
{
	...
	while (i < PASS)
	{
		vars->gradient[i].transparency = (vars->color_multiplier[0] * (int)i)
			% 255;
...
t_color	*get_color(int i, t_color *gradient, t_color *color)
{
	...
	color->transparency = gradient[i].transparency + i_scaled * (gradient[i
			+ 1].transparency - gradient[i].transparency);
	color->red = gradient[i].red + i_scaled * (gradient[i + 1].red
			- gradient[i].red);
	color->green = gradient[i].green + i_scaled * (gradient[i + 1].green
			- gradient[i].green);
	color->blue = gradient[i].blue + i_scaled * (gradient[i + 1].blue
			- gradient[i].blue);
	return (color);
}
