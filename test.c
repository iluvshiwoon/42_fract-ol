...
/*   Updated: 2024/03/01 19:05:54 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (axe == 'w')
        return(((vars->max_r - vars->min_r) * (x - 0))/(vars->view_width - 0) + vars->min_r);
    else if (axe == 'h')
        return(((vars->max_i - vars->min_i) * (x - 0))/(vars->view_height - 0) + vars->min_i);
    return 0;
}
...
int main(void) {
  ...
  vars->mlx = mlx_init();
  vars->win = mlx_new_window(vars->mlx, vars->view_width, vars->view_height, "Mandelbrot");
    vars->min_r = -4.0;
    vars->max_r = 4.0;
    vars->min_i = -4.0;
    vars->max_i = 4.0;
